...
autoload :YAML, 'yaml'
require_relative '../hash_benchmark'

module MRuby::HashBenchmark
  autoload :SimpleErb, 'mruby/hash_benchmark/simple_erb'
  autoload :HtmlBuilder, 'mruby/hash_benchmark/html_builder'
    ...
    class << self
      def config
        @config ||= (
          ...
          YAML.load(yaml, symbolize_names: true).freeze
        )
      end
